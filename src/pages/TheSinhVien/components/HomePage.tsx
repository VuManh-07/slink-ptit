...
const HomePageComponent = ()=> {
    ...
    const getBalance = async ()=>{
        await instanceCard.post("/api/payment/getBalance", {maSV: "B19DCVT247"})
           .then((res: any) =>{ 
    ...
    const updateBalance = async (amount: number)=>{
        await instanceCard.post("/api/payment/updateBalance", {maSV: "B19DCVT247", amount: amount})
        .then(res => {
    ...
    const getUser = async ()=>{ 
        await instancePtit.get("http://localhost:3040/api/user/getUser", {
            headers: { 
    ...
    const getBangDiem = async () => {
        await instancePtit.get("http://localhost:3040/api/bangdiem/getBangDiem", {
            headers: { 
              'x-access-token': localStorage.getItem("access_token")
            }
        })
    ...
    return (
        <div> 
            <h3>Home Page</h3>
            <h4>Client ID: <span>{socketCard.id}</span></h4>
            <br/>
            <h4>Balance: <span>{balance}đ</span></h4> 
